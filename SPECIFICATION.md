...
- **水深**: 魚群探知機から湖底までの距離
  - 範囲: 1m～20m
  - 刻み幅: 0.1m（191段階）
  - 入力方式: 垂直スライダー（HTML range input、step="0.1"）
  - スライダー値変換: depth = 21 - sliderValue（上が浅く、下が深い）
  - 操作方法:
    - ドラッグ操作（タッチ/マウス）
    - マウスホイール（PC、±0.1m/回）
    - ±ボタン（±0.1m/回、微調整用）
...
#### 結果カード
- **背景デザイン**
  - 77kHz（広角）: ピンク〜赤系グラデーション（ #c64dd8 → #c92a40）
  - 200kHz（狭角）: 青系グラデーション（ #1a7cc9 → #00b0c5）
  - 水深に応じて明暗が動的に変化
    - 浅い水深（1m）: 明るい（overlay opacity: 0）
    - 深い水深（20m）: 暗い（overlay opacity: 0.4）
    - CSS変数（--overlay-opacity）、::before疑似要素で実装
  - フォント色は変化せず、視認性を維持
